...
const myNFTController = {
  create: async (req, res) => {
    try {
      const item = await models.myNFT.create(req.body);
  ...
  findAll: async (req, res) => {
    try {
      const items = await models.myNFT.findAll();
      res.json(items);
    } catch (error) {
  ...
  findById: async (req, res) => {
    try {
      const item = await models.myNFT.findByPk(req.params.id);
